...
fn main() -> Result<()> {
    ...
    if args.len() >= 2 {
        ...
        for claude_path in &claude_paths {
            ...
            match monitor.load_directory(claude_path) {
                ...
                Err(e) => {
                    eprintln!("Warning: Failed to load from {}: {}", claude_path.display(), e);
                }
            }
        }
    ...
    println!("Loaded {} entries across {} sessions", 
             monitor.entry_count(), 
    ...
    if let Some(current_burn_rate) = monitor.get_current_burn_rate() {
        println!("Current session burn rate: {:.2} tokens/minute, ${:.4}/hour", 
    ...
    if let Some(projection) = monitor.project_current_usage(current_time) {
        ...
        println!("Projected additional tokens: {}", projection.projected_additional_tokens());
        println!("Projected additional cost: ${:.6}", projection.projected_additional_cost());
        println!("Projected total tokens: {}", projection.projected_total_tokens());
        println!("Projected total cost: ${:.6}", projection.projected_total_cost());
    }
    ...
    println!("Auto-detected plan: {} (based on current usage)", detected_plan.description());
    println!();
    ...
    for plan in plans {
        ...
        if current_tokens < max_tokens {
            if let Some(time_to_limit) = monitor.estimate_time_to_plan_limit(plan) {
                ...
                if days > 0 {
                    println!("  Time remaining: {}d {}h {}m", days, remaining_hours, minutes);
                } else if hours > 0 {
                    println!("  Time remaining: {}h {}m", hours, minutes);
                } else {
                    println!("  Time remaining: {}m", minutes);
                }
                println!("  Will be reached at: {}", limit_time.format("%Y-%m-%d %H:%M UTC"));
            }
        } else {
        ...
        let status_indicator = if plan == detected_plan { " ← DETECTED" } else { "" };
        println!("  Usage: [{}] {:.1}% ({}/{}){}", 
                 bar, percentage, format_number(current_tokens), format_number(max_tokens), status_indicator);
        println!();
    }
